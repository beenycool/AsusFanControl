...
namespace AsusFanControl.Core
{
    public class AsusControl : IFanController, IDisposable
    {
        ...
        private static bool _winIoExitHandlerRegistered = false;
            ...
            lock (_hardwareLock)
            {
                ...
                var initializedHere = false;
                    try
                    {
                        if (_instanceCount == 0)
                        {
                            ...
                            // ensure a single process-exit shutdown
                            if (!_winIoExitHandlerRegistered)
                            {
                                _winIoExitHandlerRegistered = true;
                    ...
                    // healthy table fan counts retrieved
                ...
                catch (Exception ex)
                {
                    ...
                    if (initializedHere)
                    {
                        ...
                        {
                            Debug.WriteLine($"[AsusControl] Error rolling back WinIo initialization: {shutdownEx.Message}");
                        }
                    }
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            if (shouldShutdown)
            {
                lock (_hardwareLock)
                {
                    ...
                    // WinIo shutdown is handled once per AppDomain on ProcessExit
                    finally
                    {
